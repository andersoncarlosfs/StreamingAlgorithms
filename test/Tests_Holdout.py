...
def ensemble_partial_fit(_train_weight_seen_by_model, _partial_fit, X, y, weight):
    if weight != 0.0:
        _train_weight_seen_by_model += weight
        _partial_fit(X, y, weight)    
        
def classifier_partial_fit(get_votes_for_instance, partial_fit, X, y, X_seen):   
    vote = get_votes_for_instance(X)
    k = np.random.poisson(self.w, POISSON_SIZE)
    if k > 0:
        partial_fit(np.asarray([X]), np.asarray([y]), np.asarray([k]), X_seen)
        
def get_elected(vote, predict):
    predict.append(max(vote, key = vote.get))
    
def get_votes(vote, combined_vote):
    if sum(vote) > 0:
        combined_vote.append(vote)
        
def assign_classifier(classifier, ensemble, i):
...
class AdaptiveRandomForest(BaseClassifier):
    ...
    def partial_fit(self, X, y, classes = None, weight = None):
        if y is not None:
            ...
            Parallel(n_jobs = -1)(delayed(ensemble_partial_fit)(self._train_weight_seen_by_model, self._partial_fit, 
                                                                X[i], y[i], weight[i]) for i in range(row_cnt))
    ...
    def _partial_fit(self, X, y, weight):
        ...
        Parallel(n_jobs = -1)(delayed(classifier_partial_fit)(self.ensemble[i].get_votes_for_instance, 
                                                              self.ensemble[i].partial_fit, X[i], y[i], 
                                                              X_seen) for i in range(self.nb_ensemble))
    ...
    def predict(self, X):
        ...
        for i in range(r):
            ...
            if votes == {}:
                ...
                Parallel(n_jobs = -1)(delayed(get_elected)(vote, predict) for vote in votes)        
    ...
    def get_votes_for_instance(self, X):
        ...
        Parallel(n_jobs = -1)(delayed(get_votes)(self.ensemble[i].get_votes_for_instance(test), 
                                                 combined_vote) for i in range(self.nb_ensemble))
    ...
    def init_ensemble(self, X):
        ...
        Parallel(n_jobs = -1)(delayed(assign_classifier)(
            ARFBaseLearner(i, ARFHoeffdingTree(nb_attributes = self.nb_attributes), self.X_seen, 
                           not self.disable_background_learner, not self.disable_drift_detection, 
                           self.drift_detection_method, self.warning_detection_method, False), 
            ensemble, i) for i in range(self.nb_ensemble))
...
eval = EvaluateHoldout(show_plot=False, pretrain_size=100, max_instances=10000)
